...
cat source/low.lex | python sort.py > source/sort.lex  ----- dict_wsp
cat source/low.lab | cut -d' ' -f2- | fmt -1 | sort | uniq > wlist
HDMan -m -w wlist -n monophones1 -i -l dlog dict dict_wsp
...
; create `monophones0` from `monophones1` + sp
; create hmmdefs
; create macros file
...
HERest -A -D -T 1 -C config -I wintri.mlf -t 250.0 150.0 1000.0 -S train.scp -H hmm10/macros -H hmm10/hmmdefs -M hmm11 triphones1
HERest -A -D -T 1 -C config -I wintri.mlf -t 250.0 150.0 1000.0 -s stats -S train.scp -H hmm11/macros -H hmm11/hmmdefs -M hmm12 triphones1

; create maketriphones.ded
HDMan -A -D -T 1 -b sp -n fulllist0 -g maketriphones.ded -l flog dict-tri dict_wsp
...
; create n-gram
; cat daily.lab | cut -d' ' -f2- > daily_clean.lab
; remove <> from daily_clean.lab
; mkdir ngram.0
; LNewMap -f WFC ngram empty.wmap 
; LGPrep -T 1 -a 100000 -b 200000 -d ngram.0 -n 4 -s "ngram" empty.wmap daily_clean.lab
